 ...
 * 配置文件路径 **Assets/Framework//EasyAssets/Resources/EasyAssetsConfig.asset**。 
 1. **压缩资源配置**
 如下图所示，可以选择是否开启资源压缩，并指定解压密码。如果开启压缩，会将AssetBundle压缩成zip格式的压缩包。
  <div align="left">
   <img src="../../../picture/pic_434.png" height=180 alt="压缩资源配置">
 	</div>
 ### 1.3.3 特殊文件说明
 ...
 * 使用新安装包**覆盖安装app**时，若**随包的资源版本号**更新，会以随包的资源版本号检查是否需要下载资源。

    例如：用户安装的app资源版本号为***1.1.1(2)***，安装包的随包资源版本号为***1.2.0(1)***,调用**BundleCheck.CheckUpdateFromRemote**（不指定特定版本号）时，会以***1.2.0(1)***进行判断，并更新资源。

 * 跨版本号进行资源热更新时（不覆盖安装app的情况下），需要**手动传入**想要进行对比的版本号(**通常需要主动从逻辑服务器获取当前版本对应的资源版本号**)，然后会下载服务器对应该版本号的路径下的bundleinfo进行对比。当前App版本对应哪一资源版本号，需要**自行实现**。
---
